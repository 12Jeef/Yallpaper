...
fn project(coord: vec2f) -> vec3f {
  ...
  let warp_mult = 1.25 - 1.5 * (r / length(uniforms.resolution * 0.5 - origin) - 0.75);
...
const RANDOM_SCALE: vec4f = vec4f(.1031, .1030, .0973, .1099);`,t=1,n=document.querySelector(`#app > canvas#sky`);if(!(n instanceof HTMLCanvasElement))throw document.body.innerHTML=`Canvas Sky: Element not found`,Error(`Canvas Sky: Element not found`);var r=n,i=document.querySelector(`#app > canvas#grass`);if(!(i instanceof HTMLCanvasElement))throw document.body.innerHTML=`Canvas Grass: Element not found`,Error(`Canvas Grass: Element not found`);var a=i;function o(e,n){function r(){e.width=Math.floor(window.innerWidth*t),e.height=Math.floor(window.innerHeight*t),e.style.width=`${window.innerWidth}px`,e.style.height=`${window.innerHeight}px`,n?.()}r(),new ResizeObserver(r).observe(document.body)}async function s(){let n=await navigator.gpu.requestAdapter();if(!n)throw Error(`Canvas Sky: Failed to request GPU adapter`);let i=await n.requestDevice(),a=r.getContext(`webgpu`);if(!a)throw Error(`Canvas Sky: Failed to get WebGPU context`);let s=a,c=navigator.gpu.getPreferredCanvasFormat();s.configure({device:i,format:c,alphaMode:`opaque`}),o(r);let l=i.createShaderModule({code:e}),u=i.createShaderModule({code:`@vertex fn vs_main(@builtin(vertex_index) idx: u32) -> @builtin(position) vec4f {
  ...
  }`}),d=i.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.FRAGMENT,buffer:{type:`uniform`}}]}),f=i.createPipelineLayout({bindGroupLayouts:[d]}),p=i.createRenderPipeline({layout:f,vertex:{module:u,entryPoint:`vs_main`},fragment:{module:l,entryPoint:`fs_main`,targets:[{format:c}]},primitive:{topology:`triangle-strip`}}),m=i.createBuffer({size:16,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),h=i.createBindGroup({layout:d,entries:[{binding:0,resource:{buffer:m}}]}),g=new ArrayBuffer(16),_=new Float32Array(g),v=Date.now();async function y(){let e=(Date.now()-v)/1e3;_[0]=r.width*(2/t),_[1]=r.height*(2/t),_[2]=e,_[3]=t,i.queue.writeBuffer(m,0,g);let n=s.getCurrentTexture().createView(),a=i.createCommandEncoder(),o=a.beginRenderPass({colorAttachments:[{view:n,loadOp:`clear`,clearValue:{r:0,g:0,b:0,a:1},storeOp:`store`}]});o.setPipeline(p),o.setBindGroup(0,h),o.draw(4,1,0,0),o.end(),i.queue.submit([a.finish()]),await i.queue.onSubmittedWorkDone()}(async()=>{for(;;)await new Promise(e=>setTimeout(e,1/15*1e3)),y()})()}async function c(){let e=a.getContext(`2d`);if(!e)throw Error(`Canvas Grass: Failed to get 2D context`);let n=e;function r(e,t){return Math.sign(e)*Math.abs(e)**+t}function i(e,t,n){return e+(t-e)*n}function s(e,t,n){return[i(e[0],t[0],n),i(e[1],t[1],n),i(e[2],t[2],n)]}function c(e){let[t,n,r]=e;return`rgb(${t*255}, ${n*255}, ${r*255})`}let l=[],u=0,d=[];for(let e=0;e<8;e++)d.push({pos:[r(Math.random()-.25,1.5)+.25,Math.random()],r:Math.random(),seed:[Math.random(),Math.random(),Math.random(),Math.random()]});for(let e=0;e<2;e++)d.push({pos:[i(1.5,2.5,Math.random()),i(-.75,.25,Math.random())],r:Math.random(),seed:[Math.random(),Math.random(),Math.random(),Math.random()]});let f=75*t;function p(){l.splice(0,l.length);let e=a.height*.05,n=a.height*.2,r=30*t,o=0;for(let t=0;t<n+f;t+=r*(.02+4*(t/n)**1.5)){o++;for(let s=-(Math.random()+2)*r;s<a.width+2*r;s+=r*i(.9,1.1,Math.random())){let r=s/a.width,c=i(e,n,.5+.5*Math.sin(2.0943951024+-3.6651914292*r)),u=a.height-c+t+30*i(-1,1,Math.random());l.push({pos:[s,u,o],size:f*i(.75,1.25,Math.random()),seed:Math.random()})}}u=o}o(a,()=>{p()});let m=Date.now();function h(){let e=(Date.now()-m)/1e3;n.clearRect(0,0,a.width,a.height);for(let t of l){let{pos:[r,o,l],size:d,seed:f}=t,p=r/a.width,m=i(15,45,.5+.5*Math.sin(5*p+2*f+.5*e))*(Math.PI/180),h=r-Math.sin(m)*d,g=o-Math.cos(m)*d,_=n.createLinearGradient(r,o,h,g),v=(l/u)**2;_.addColorStop(1,c(s(s([.075,.05,.3],[0,0,.1],v),[0,0,.3],p*.5))),_.addColorStop(0,c(s(s([.1,.1,.5],[.05,.02,.15],v),[0,0,.15],p*.5))),n.fillStyle=_,n.beginPath(),n.moveTo(r-d*.5,a.height),n.lineTo(r-d*.5,o),n.lineTo(r-d*.25,o),n.lineTo(h,g),n.lineTo(r+d*.25,o),n.lineTo(r+d*.5,o),n.lineTo(r+d*.5,a.height),n.closePath(),n.fill()}let r=`#1cff77`;for(let o of d){let{pos:[s,c],r:l,seed:[u,d,f,p]}=o,m=i(.05,.5,s)*a.width,h=(1-i(.2,.4,c))*a.height,g=i(.01,.0075,l)*Math.hypot(a.width,a.height),_=(.5+.5*Math.sin(u*2*Math.PI+i(1.5,3.5,u)*e))**2,v=p*2*Math.PI,y=v+i(.5,5,d)*e,b=v+i(.5,5,f)*e,x=m+Math.cos(y)*g,S=h+Math.sin(b)*g;n.globalAlpha=_,n.fillStyle=r,n.beginPath(),n.arc(x,S,i(1.5,2.5,_)*t,0,2*Math.PI),n.fill(),n.fillStyle=`#ffffff`,n.beginPath(),n.arc(x,S,i(0,1.5,_)*t,0,2*Math.PI),n.fill(),n.globalAlpha=_*.25;let C=i(0,30,_)*t,w=n.createRadialGradient(x,S,0,x,S,C);w.addColorStop(0,r),w.addColorStop(.5,`#1cff7744`),w.addColorStop(1,`#1cff7700`),n.fillStyle=w,n.beginPath(),n.arc(x,S,C,0,2*Math.PI),n.fill(),n.globalAlpha=1}}(async()=>{for(;;)await new Promise(e=>setTimeout(e,1/30*1e3)),h()})()}async function l(){try{await s()}catch(e){document.body.innerHTML=`Canvas Sky: Error: ${e instanceof Error?e.message:String(e)}`;return}try{await c()}catch(e){document.body.innerHTML=`Canvas Grass: Error: ${e instanceof Error?e.message:String(e)}`;return}}l();